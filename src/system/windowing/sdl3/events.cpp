...
SDL_Window *sdl_window_from_event(const SDL_Event &event)
{
	    ...
	    event.type <= SDL_EVENT_WINDOW_LAST)
	{
		...
		switch (event.type)
		{
			...
			case SDL_EVENT_KEY_UP:   return SDL_GetWindowFromID(event.key.windowID);
			...
			case SDL_EVENT_USER: return SDL_GetWindowFromID(event.user.windowID);
...
lak::mod_key get_mod_key_state()
{
	...
	if ((sdl_mod & SDL_KMOD_LSHIFT) != 0) mod = mod | lak::mod_key::lshift;
	if ((sdl_mod & SDL_KMOD_RSHIFT) != 0) mod = mod | lak::mod_key::rshift;
	if ((sdl_mod & SDL_KMOD_LCTRL) != 0) mod = mod | lak::mod_key::lctrl;
	if ((sdl_mod & SDL_KMOD_RCTRL) != 0) mod = mod | lak::mod_key::rctrl;
	if ((sdl_mod & SDL_KMOD_LALT) != 0) mod = mod | lak::mod_key::lalt;
	if ((sdl_mod & SDL_KMOD_RALT) != 0) mod = mod | lak::mod_key::ralt;
	if ((sdl_mod & SDL_KMOD_LGUI) != 0) mod = mod | lak::mod_key::lsuper;
	if ((sdl_mod & SDL_KMOD_RGUI) != 0) mod = mod | lak::mod_key::rsuper;
	return mod;
}
                     ...
                     const lak::window_handle *window = nullptr)
{
	...
	switch (sdl_event.type)
	{
		...
		case SDL_EVENT_WINDOW_PIXEL_SIZE_CHANGED:
		{
			...
			*event =
			  lak::event(lak::event_type::window_changed,
			             window,
			             lak::move(platform_event),
			             lak::window_event{
		...
		{
			auto str =
			  lak::as_u8string(lak::astring_view::from_c_str(sdl_event.text.text));
		...
		case SDL_EVENT_KEY_DOWN:
		{
			uint32_t scancode = sdl_event.key.scancode;
		...
		case SDL_EVENT_DROP_FILE:
		{
			*event = lak::event(
			  ...
			  lak::dropfile_event{
			    ...
			    .path     = lak::u8string(
            reinterpret_cast<const char8_t *>(sdl_event.drop.data))});
		}
		break;
...
bool handle_next_event(lak::event *event, SDL_EventAction action)
{
	SDL_PumpEvents();
	if (SDL_Event e;
	    SDL_PeepEvents(&e, 1, action, SDL_EVENT_FIRST, SDL_EVENT_LAST) > 0)
	{
