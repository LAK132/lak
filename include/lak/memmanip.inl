...
template<typename T>
lak::span<lak::copy_const_t<T, byte_t>, sizeof(T)> lak::as_bytes<T>(T *v)
{
...
template<typename T>
force_inline void lak::memcpy<T>(T *dst, const T *src)
{
	lak::memcpy(lak::as_bytes(dst), lak::as_bytes(src));
}

template<typename T>
force_inline void lak::memmove<T>(T *dst, const T *src)
{
	lak::memmove(lak::as_bytes(dst), lak::as_bytes(src));
}

template<typename T>
force_inline void lak::bzero<T>(T *dst)
{
	lak::fill<byte_t>(lak::as_bytes(dst), byte_t(0));
}

template<size_t CHUNK_SIZE>
void lak::byte_swap<CHUNK_SIZE>(lak::span<byte_t> v)
{
...
template<typename T>
void lak::byte_swap<T>(lak::span<T> v)
{
...
TO lak::bit_cast<TO, FROM>(const FROM &from)
{
	TO result;
